...
### Cloud Password Reset (`/cloud-password-reset`)

Reset the password of an Entra ID **cloud-only** account - one with no on-premises AD object, and
therefore unreachable from the AD tooling L2 already has. Almost every target is an admin or
tactical account; resetting a Global Administrator is the module's normal operating mode.

- **The operator does not choose where the password goes.** The module reads the `employeeId` off
  the target account, finds the one directory user carrying it, and emails the new password to
  that person's mailbox. There is no address field on the page.
- **The operator normally never sees the password.** It is generated by the app, written to Entra
  and mailed. A separate `CloudPasswordResetReveal` permission shows it on screen instead, for an
  account whose owner cannot be determined - the only route by which an operator can learn one.
- Refuses, with its reason audited, when the account is synced or a guest, when the employee ID is
  missing or matches nobody, when it matches **more than one** person, when the owner has no
  mailbox, or when the directory could not be searched. None falls back to another destination.
- Generated passwords are 18-32 character passphrases with a measured 60-bit entropy floor; after
  100 attempts that fail the floor the generator refuses rather than issuing something weaker.
- **Force password change at next sign-in is checked by default** - the password travels by email,
  so forcing a change makes it a one-time handover. Clearable per reset for an account whose
  sign-in path cannot service a change prompt.
- A send failure after the write discards the password rather than showing it, logs at Critical
  and audits `CloudPasswordReset_DeliveryFailed`. Running the reset again issues a new one.
- **Requires:** its own Graph app registration with `User.Read.All`,
  `User-PasswordProfile.ReadWrite.All` and `RoleManagement.Read.Directory`, and its own Delinea
  record. Ships disabled.
- Section access keys: `CloudPasswordReset`, `CloudPasswordResetReveal`
- Full detail: `docs/CloudPasswordReset.md`
