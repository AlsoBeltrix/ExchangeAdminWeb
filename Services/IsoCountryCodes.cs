...
/// written by this app via <c>Set-ADUser</c> is identical to one written by ADUC or the
...
public static class IsoCountryCodes
{
    ...
    /// <summary>Number of entries in the table. Used by the integrity test.</summary>
    public static int Count => NumericByAlpha2.Count;

    /// <summary>
    /// Returns the ISO 3166-1 numeric code for an alpha-2 code, or null if the alpha-2 is
    /// not a recognized country. Case-insensitive.
    /// </summary>
    public static int? GetNumeric(string alpha2)
    {
        if (string.IsNullOrWhiteSpace(alpha2))
            return null;
        return NumericByAlpha2.TryGetValue(alpha2.Trim(), out var n) ? n : null;
    }
}
