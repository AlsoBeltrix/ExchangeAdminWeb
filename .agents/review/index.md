...
| pps-1 | HIGH | Mailbox/Calendar bulk CSV cannot service, and the on-prem branch writes without a fresh protection check | `[ ]` | | codex/gpt-5.5-dzs/xhigh/std (base `b351005..HEAD`) — bulk calls the back-compat `ValidateTargetMailboxAsync` overload that by construction never services, so a servicer is allowed one row at a time and refused in a CSV; separately `ExecuteOnPrem` re-checks authorization but not protection after the confirmation dialog, violating "immediately before the write" and auditing no override. **NOT YET FIXED.** |
| pps-2 | HIGH | Page-level gates block the servicer before the serviced service gate can run | `[ ]` | | codex (same pass) — `ADAttributeEditor.razor:313` sets `protectedBlocked` and hides the edit UI with no servicer consultation, so the serviced save gate is unreachable from the page; undo preview gets no principal while execute does. **This is the Emergency Disable two-gate shape I recorded and then failed to apply to the other module that has it.** Same class as the 2026-08-06 unreachable-capability defect. **NOT YET FIXED.** |
| pps-3 | HIGH | Serviced notes computed and then discarded, losing the audit record of the override | `[ ]` | | codex (same pass) — `ADAttributeEditorUndoService.cs:180` evaluates `NoteFor(...) is null` inside a boolean and drops the note on the allow path; Emergency Disable keeps its note in the operation trace and out of the `AuditService` event. **The helper returns a nullable note precisely so permission and record cannot be separated, and using it as a bare null test defeats that by design.** Consider a source-level guard forbidding that call shape. **NOT YET FIXED.** |
